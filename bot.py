...
async def setup(ctx):
    ...
    for name in BOSS_DATA:
        ...
        if role is None:
            ...
            await ctx.send('A {role} has been created.'.format(role=discord.role.mention(object=name)))
        else:
            await ctx.send('A {role} role already exists.'.format(role=discord.role.mention(object=name)))
